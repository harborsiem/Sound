 ...
 */
    public sealed class Track {
         ...
         */
        private readonly List<MidiEvent> eventsList = new List<MidiEvent>();

        /**
         * Use a hashset to detect duplicate events in add(MidiEvent).
         */
        private readonly HashSet<MidiEvent> set = new HashSet<MidiEvent>();

        private readonly MidiEvent eotEvent;
         ...
         */
        public bool add(MidiEvent evnt) {
            if (evnt == null) {
                return false;
            }
            lock (eventsList) {

                if (!set.Contains(evnt)) {
                    int eventsCount = eventsList.Count;

                    // get the last event
                    MidiEvent lastEvent = null;
                    if (eventsCount > 0) {
                        lastEvent = eventsList[eventsCount - 1];
                    }
                    ...
                    if (MidiUtils.isMetaEndOfTrack(evnt.getMessage())) {
                        ...
                        if (evnt.getTick() > eotEvent.getTick()) {
                            eotEvent.setTick(evnt.getTick());
                        }
                        return true;
                    }
                    ...
                    int i = eventsCount;
                    for (; i > 0; i--) {
                        if (evnt.getTick() >= ((MidiEvent)eventsList[i - 1]).getTick()) {
                            break;
                        }
                    }
                    if (i == eventsCount) {
                        ...
                        eventsList[eventsCount - 1] = evnt;
                        // set new time of eot, if necessary
                        if (eotEvent.getTick() < evnt.getTick()) {
                            eotEvent.setTick(evnt.getTick());
                        }
                        // add eot again at the end
                        eventsList.Add(eotEvent);
                    } else {
                        eventsList.Insert(i, evnt);
                    }
                    return true;
                }
            }
         ...
         */
        public bool remove(MidiEvent evnt) {
            ...
            lock (eventsList) {
                return set.Remove(evnt) && eventsList.Remove(evnt);
            }
        }
        ...
        public MidiEvent get(int index) {
            try {
                ...
                throw; //new ArrayIndexOutOfBoundsException(ioobe.Message);
            }
        }
        ...
        public long ticks() {
            ...
            lock (eventsList) {
                if (eventsList.Count > 0) {
                    ret = eventsList[eventsList.Count - 1].getTick();
                }
            }
            return ret;
        }

        private class ImmutableEndOfTrack : MetaMessage {
            public ImmutableEndOfTrack()
                : base(new byte[3]) {
                data[0] = (byte)META;
            ...
            public override void setMessage(int type, byte[] data, int length) {
