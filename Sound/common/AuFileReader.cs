 ...
 */
    public sealed class AuFileReader : SunFileReader {
        ...
        internal override StandardFileFormat getAudioFileFormatImpl(Stream stream) {
            ...
            long /* unsigned int */ dataSize = dis.ReadInt32() & 0xffffffffL;
            int auType = dis.ReadInt32();
            int sampleRate = dis.ReadInt32();
            if (sampleRate <= 0) {
                throw new UnsupportedAudioFileException("Invalid sample rate");
            }
            int channels = dis.ReadInt32();
            if (channels <= 0) {
            ...
            byte[] b = dis.ReadBytes(headerSize - AuFileFormat.AU_HEADERSIZE);
