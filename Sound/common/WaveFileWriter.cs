 ...
 */
    public sealed class WaveFileWriter : SunFileWriter {

        /**
         * Constructs a new WaveFileWriter object.
         */
        public WaveFileWriter()
            : base(new AudioFileFormat.Type[] { AudioFileFormat.Type.WAVE }) {
        }
        ...
        public override int write(AudioInputStream stream, AudioFileFormat.Type fileType, Stream output) {
            ...
            return writeWaveFile(stream, waveFileFormat, output);
        }
        ...
        public override int write(AudioInputStream stream, AudioFileFormat.Type fileType, FileInfo output) {
            ...
            if (waveFileFormat.getByteLength() == AudioSystem.NOT_SPECIFIED) {
                ...
                using (FileStream fs = new FileStream(output.FullName, FileMode.Open, FileAccess.ReadWrite)) {
                    ...
                    raf.Write(big2little(riffLength));
                    // skip WAVE magic, fmt_ magic, fmt_ length, fmt_ chunk, data magic
                    raf.BaseStream.Position += (4 + 4 + 4 + WaveFileFormat.getFmtChunkSize(waveFileFormat.getWaveType()) + 4);
                    raf.Write(big2little(dataLength));
                    // that's all
                    raf.Close();
                }
            }
...
#pragma warning restore 0168
        ...
        private int writeWaveFile(Stream input, WaveFileFormat waveFileFormat, Stream output) {

            int bytesRead = 0;
            int bytesWritten = 0;
            Stream fileStream = getFileStream(waveFileFormat, input);
        ...
        private Stream getFileStream(WaveFileFormat waveFileFormat, Stream audioStream) {
            ...
            using (MemoryStream baos = new MemoryStream()) {
                ...
                dos.Write(riffMagic);
                dos.Write(big2little(riffLength));
                dos.Write(waveMagic);
                dos.Write(fmtMagic);
                dos.Write(big2little(fmtLength));
                dos.Write(big2littleShort(wav_type));
                dos.Write(big2littleShort(channels));
                dos.Write(big2little(sampleRate));
                dos.Write(big2little(avgBytesPerSec));
                dos.Write(big2littleShort(blockAlign));
                dos.Write(big2littleShort(sampleSizeInBits));
                ...
                dos.Write(dataMagic);
                dos.Write(big2little(dataLength));
                header = baos.ToArray();
            }
