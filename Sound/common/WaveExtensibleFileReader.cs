 ...
 */
    public sealed class WaveExtensibleFileReader : SunFileReader {
        ...
        private sealed class GUID {
            ...
            internal GUID(long i1, int s1, int s2, int x1, int x2, int x3, int x4,
            ...
            public static GUID read(RIFFReader riff) {
                GUID d = new GUID();
        ...
        private static readonly String[] channelnames = { "FL", "FR", "FC", "LF",
            "BL",
            "BR", // 5.1
            "FLC", "FLR", "BC", "SL", "SR", "TC", "TFL", "TFC", "TFR", "TBL",
            "TBC", "TBR" };

        private static readonly String[] allchannelnames = { "w1", "w2", "w3", "w4", "w5",
        ...
        private static readonly GUID SUBTYPE_PCM = new GUID(0x00000001, 0x0000, 0x0010,
                0x80, 0x00, 0x00, 0xaa, 0x00, 0x38, 0x9b, 0x71);

        private static readonly GUID SUBTYPE_IEEE_FLOAT = new GUID(0x00000003, 0x0000,
        ...
        internal override StandardFileFormat getAudioFileFormatImpl(Stream stream) {
            ...
            if (s_channelmask != null)
                p["channelOrder"] = s_channelmask; // p.Add("channelOrder", s_channelmask);
            if (channelMask != 0)
                p["channelMask"] = channelMask; // p.Add("channelMask", channelMask);
            // validBitsPerSample is only informational for PCM data,
            // data is still encode according to SampleSizeInBits.
            p["validBitsPerSample"] = validBitsPerSample; // p.Add("validBitsPerSample", validBitsPerSample);
            ...
            if (subFormat.Equals(SUBTYPE_PCM)) {
                if (bits == 8) {
                    ...
                    audioformat = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, samplerate,
                            bits, channels, framesize, samplerate, false, p);
                }
            } else if (subFormat.Equals(SUBTYPE_IEEE_FLOAT)) {
                audioformat = new AudioFormat(AudioFormat.Encoding.PCM_FLOAT,
