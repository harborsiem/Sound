 ...
 */
    internal sealed class FastShortMessage : ShortMessage {
        private int packedMsg;

        internal FastShortMessage(int packedMsg) {
        ...
        internal FastShortMessage(ShortMessage msg) {
            this.packedMsg = msg.getStatus()
                | (msg.getData1() << 8)
                | (msg.getData2() << 16);
        }

        internal int getPackedMsg() {
            return packedMsg;
        }
        ...
        public override void setMessage(int status) {
            // check for valid values
            int dataLength = getDataLength(status); // can throw InvalidMidiDataException
            if (dataLength != 0) {
                base.setMessage(status); // throws Exception
            }
            packedMsg = (packedMsg & 0xFFFF00) | (status & 0xFF);
        }


        public override void setMessage(int status, int data1, int data2) {
            getDataLength(status); // can throw InvalidMidiDataException
            packedMsg = (status & 0xFF) | ((data1 & 0xFF) << 8) | ((data2 & 0xFF) << 16);
        }


        public override void setMessage(int command, int channel, int data1, int data2) {
        ...
        public override int getData1() {
            return (packedMsg & 0xFF00) >> 8;
        }

        public override int getData2() {
