 ...
 */
    public sealed class AudioSystem {
        ...
        public static Mixer.Info[] getMixerInfo() {
            ...
            IList<Mixer.Info> infos = getMixerInfoList();
            Mixer.Info[] allInfos = ((List<Mixer.Info>)infos).ToArray();
            return allInfos;
        }
         ...
         * @see #getMixerInfo
         */
        public static IMixer getMixer(Mixer.Info info) {
            foreach (MixerProvider provider in getMixerProviders()) {
                try {
            ...
            throw new ArgumentException(
                     String.Format("Mixer not supported: {0}", info));
        }
        ...
        public static Line.Info[] getSourceLineInfo(Line.Info info) {
            ...
            List<Line.Info> list = new List<Line.Info>();
...
#pragma warning restore 0219
            ...
            Line.Info[] returnedArray = new Line.Info[list.Count];

            for (int i = 0; i < returnedArray.Length; i++) {
                returnedArray[i] = list[i];
            }
...
#pragma warning restore 0219
         ...
         *         any installed mixer
         */
        public static ILine getLine(Line.Info info) {
            LineUnavailableException lue = null;
            IList<MixerProvider> providers = getMixerProviders();


            // 1: try from default mixer for this line class
            try {
                IMixer mixer = getDefaultMixer(providers, info);
            ...
            for (int i = 0; i < providers.Count; i++) {
                MixerProvider provider = providers[i];
                Mixer.Info[] infos = provider.getMixerInfo();

                for (int j = 0; j < infos.Length; j++) {
                    try {
                        IMixer mixer = provider.getMixer(infos[j]);
         ...
         * @since 1.5
         */
        public static IClip getClip() {
            AudioFormat format = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED,
                                 AudioSystem.NOT_SPECIFIED,
                                 16, 2, 4,
                                 AudioSystem.NOT_SPECIFIED, true);
            DataLine.Info info = new DataLine.Info(typeof(IClip), format);
            return (IClip)AudioSystem.getLine(info);
        }
         ...
         * @since 1.5
         */
        public static IClip getClip(Mixer.Info mixerInfo) {
            AudioFormat format = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED,
                                 AudioSystem.NOT_SPECIFIED,
                                 16, 2, 4,
                                 AudioSystem.NOT_SPECIFIED, true);
            DataLine.Info info = new DataLine.Info(typeof(IClip), format);
            IMixer mixer = AudioSystem.getMixer(mixerInfo);
            return (IClip)mixer.getLine(info);
        }
         ...
         * @since 1.5
         */
        public static ISourceDataLine getSourceDataLine(AudioFormat format) {
            DataLine.Info info = new DataLine.Info(typeof(ISourceDataLine), format);
            return (ISourceDataLine)AudioSystem.getLine(info);
        }
        ...
        public static ISourceDataLine getSourceDataLine(AudioFormat format,
            ...
            DataLine.Info info = new DataLine.Info(typeof(ISourceDataLine), format);
            IMixer mixer = AudioSystem.getMixer(mixerinfo);
            return (ISourceDataLine)mixer.getLine(info);
        }
         ...
         * @since 1.5
         */
        public static ITargetDataLine getTargetDataLine(AudioFormat format) {

            DataLine.Info info = new DataLine.Info(typeof(ITargetDataLine), format);
            return (ITargetDataLine)AudioSystem.getLine(info);
        }
        ...
        public static ITargetDataLine getTargetDataLine(AudioFormat format,
            ...
            DataLine.Info info = new DataLine.Info(typeof(ITargetDataLine), format);
            IMixer mixer = AudioSystem.getMixer(mixerinfo);
            return (ITargetDataLine)mixer.getLine(info);
        }
        ...
        public static AudioFormat.Encoding[] getTargetEncodings(AudioFormat.Encoding sourceEncoding) {
            ...
            IList<FormatConversionProvider> codecs = getFormatConversionProviders();
            List<AudioFormat.Encoding> encodings = new List<AudioFormat.Encoding>();
        ...
        public static AudioFormat.Encoding[] getTargetEncodings(AudioFormat sourceFormat) {
            ...
            IList<FormatConversionProvider> codecs = getFormatConversionProviders();
            List<AudioFormat.Encoding> encs = new List<AudioFormat.Encoding>();

            // gather from all the codecs
            foreach (FormatConversionProvider codec in codecs) {
                encs.AddRange(codec.getTargetEncodings(sourceFormat));
            }
         ...
         */
        public static bool isConversionSupported(AudioFormat.Encoding targetEncoding, AudioFormat sourceFormat) {
        ...
        public static AudioInputStream getAudioInputStream(AudioFormat.Encoding targetEncoding,
            ...
            throw new ArgumentException("Unsupported conversion: " + targetEncoding + " from " + sourceStream.getFormat());
        }
        ...
        public static AudioFormat[] getTargetFormats(AudioFormat.Encoding targetEncoding, AudioFormat sourceFormat) {
            ...
            foreach (FormatConversionProvider codec in codecs) {
                ...
                foreach (AudioFormat format in elements) {
                    formats.Add(format);
        ...
        public static AudioInputStream getAudioInputStream(AudioFormat targetFormat,
            ...
            throw new ArgumentException("Unsupported conversion: " + targetFormat + " from " + sourceStream.getFormat());
        }
         ...
         */
        public static AudioFileFormat getAudioFileFormat(Stream stream) {
            if (stream == null)
                throw new ArgumentNullException(nameof(stream));

            foreach (AudioFileReader reader in getAudioFileReaders()) {
                try {
         ...
         */
        public static AudioFileFormat getAudioFileFormat(Uri url) {
            if (url == null)
         ...
         */
        public static AudioFileFormat getAudioFileFormat(FileInfo file) {
            if (file == null)
         ...
         * @see InputStream#mark
         */
        public static AudioInputStream getAudioInputStream(Stream stream) {
            if (stream == null)
                throw new ArgumentNullException(nameof(stream));
         ...
         * @throws NullPointerException if {@code url} is {@code null}
         */
        public static AudioInputStream getAudioInputStream(Uri url) {
            if (url == null)
                throw new ArgumentNullException(nameof(url));
         ...
         * @throws NullPointerException if {@code file} is {@code null}
         */
        public static AudioInputStream getAudioInputStream(FileInfo file) {
            if (file == null)
                throw new ArgumentNullException(nameof(file));
        ...
        public static AudioFileFormat.Type[] getAudioFileTypes() {
            IList<AudioFileWriter> providers = getAudioFileWriters();
            HashSet<AudioFileFormat.Type> returnTypesSet = new HashSet<AudioFileFormat.Type>();

            for (int i = 0; i < providers.Count; i++) {
                AudioFileWriter writer = providers[i];
                AudioFileFormat.Type[] fileTypes = writer.getAudioFileTypes();
                for (int j = 0; j < fileTypes.Length; j++) {
                    returnTypesSet.Add(fileTypes[j]);
                }
            }
            AudioFileFormat.Type[] returnTypes = new AudioFileFormat.Type[returnTypesSet.Count];
            returnTypesSet.CopyTo(returnTypes);
            return returnTypes;
        }
         ...
         */
        public static bool isFileTypeSupported(AudioFileFormat.Type fileType) {
            if (fileType == null)
        ...
        public static int write(AudioInputStream stream, AudioFileFormat.Type fileType,
            ...
            foreach (AudioFileWriter writer in getAudioFileWriters()) {
                try {
                    return writer.write(stream, fileType, output);
                } catch (ArgumentException) {
                    // thrown if this provider cannot write the stream, try next
                }
            }
            // "File type " + type + " not supported."
            throw new ArgumentException("could not write audio file: file type not supported: "
                + fileType);
        }
         ...
         */
        private static IList<MixerProvider> getMixerProviders() {
            //return (IList<MixerProvider>)getProviders(typeof(MixerProvider));
            IList providers = getProviders(typeof(MixerProvider));
            List<MixerProvider> result = new List<MixerProvider>();
            foreach (var provider in providers) {
                result.Add((MixerProvider)provider);
            }
            return result;
        }
         ...
         */
        private static IList<FormatConversionProvider> getFormatConversionProviders() {
            //return (IList<FormatConversionProvider>)getProviders(typeof(FormatConversionProvider));
            IList providers = getProviders(typeof(FormatConversionProvider));
            List<FormatConversionProvider> result = new List<FormatConversionProvider>();
            foreach (var provider in providers) {
                result.Add((FormatConversionProvider)provider);
            }
            return result;
        }
         ...
         */
        private static IList<AudioFileReader> getAudioFileReaders() {
            //return (IList<AudioFileReader>)getProviders(typeof(AudioFileReader));
            IList providers = getProviders(typeof(AudioFileReader));
            List<AudioFileReader> result = new List<AudioFileReader>();
            foreach (var provider in providers) {
                result.Add((AudioFileReader)provider);
            }
            return result;
        }
         ...
         *         available on the system, an empty List is returned.
         */
        private static IList<AudioFileWriter> getAudioFileWriters() {
            //return (IList<AudioFileWriter>)getProviders(typeof(AudioFileWriter));
            IList providers = getProviders(typeof(AudioFileWriter));
            List<AudioFileWriter> result = new List<AudioFileWriter>();
            foreach (var provider in providers) {
                result.Add((AudioFileWriter)provider);
            }
            return result;
        }
         ...
         *         mixer found
         */
        private static IMixer getDefaultMixer(IList<MixerProvider> providers, Line.Info info) {
            Type lineClass = info.getLineClass();
         ...
         */
        private static IMixer getNamedMixer(String mixerName,
                           MixerProvider provider,
                           Line.Info info) {
            Mixer.Info[] infos = provider.getMixerInfo();
            for (int i = 0; i < infos.Length; i++) {
                if (infos[i].getName().Equals(mixerName)) {
                    IMixer mixer = provider.getMixer(infos[i]);
                    if (isAppropriateMixer(mixer, info, false)) {
                        return mixer;
                    }
                }
        ...
        private static IMixer getNamedMixer(String mixerName,
            ...
            for (int i = 0; i < providers.Count; i++) {
                MixerProvider provider = providers[i];
                IMixer mixer = getNamedMixer(mixerName, provider, info);
                if (mixer != null) {
                    return mixer;
                }
            }
         ...
         */
        private static IMixer getFirstMixer(MixerProvider provider,
                           Line.Info info,
                           bool isMixingRequired) {
            Mixer.Info[] infos = provider.getMixerInfo();
            for (int j = 0; j < infos.Length; j++) {
                IMixer mixer = provider.getMixer(infos[j]);
         ...
         */
        private static bool isAppropriateMixer(IMixer mixer,
                              Line.Info lineInfo,
                              bool isMixingRequired) {
            if (!mixer.isLineSupported(lineInfo)) {
                return false;
            }
            Type lineClass = lineInfo.getLineClass();
            if (isMixingRequired
                && (typeof(ISourceDataLine).IsAssignableFrom(lineClass) ||
                typeof(IClip).IsAssignableFrom(lineClass))) {
         ...
         * @see #getMixerInfo()
         */
        private static IList<Mixer.Info> getMixerInfoList() {
            IList<MixerProvider> providers = getMixerProviders();
         ...
         * @see #getMixerInfo()
         */
        private static IList<Mixer.Info> getMixerInfoList(IList<MixerProvider> providers) {
            List<Mixer.Info> infos = new List<Mixer.Info>();

            Mixer.Info[] someInfos = null;  // per-mixer
#pragma warning disable 0219
            Mixer.Info[] allInfos = null;  // for all mixers
#pragma warning restore 0219

            for (int i = 0; i < providers.Count; i++) {
                someInfos = providers[i].getMixerInfo();
