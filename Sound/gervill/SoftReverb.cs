 ...
 */
    public sealed class SoftReverb : ISoftAudioProcessor {
        ...
        private sealed class AllPass {
            ...
            private readonly int delaybuffersize;
            private int rovepos = 0;
            private float feedback;

            internal AllPass(int size) {
            ...
            public void processReplace(float[] input, float[] output) {
                ...
                for (int i = 0; i < len; i++) {
                    ...
                    output[i] = delayout - input0;
                    delaybuffer[rovepos] = input0 + delayout * feedback;
                    if (++rovepos == delaybuffersize)
                        rovepos = 0;
                }
                this.rovepos = rovepos;
        ...
        private sealed class Comb {
            ...
            public void processMix(float[] input, float[] output) {
                int len = input.Length;
                int delaybuffersize = this.delaybuffersize;
                int rovepos = this.rovepos;
                ...
                for (int i = 0; i < len; i++) {
                    ...
                    output[i] += delayout;
                    delaybuffer[rovepos] = input[i] + filtertemp;
                    if (++rovepos == delaybuffersize)
                        rovepos = 0;
                }
        ...
        private float[] pre2;
        private float[] pre3;
        private bool denormal_flip = false;
        ...
        public void setMixMode(bool mix) {
            this.mix = mix;
        }
        ...
        public void processAudio() {
            bool silent_input = this.inputA.isSilent();
            if (!silent_input)
                silent = false;
            if (silent) {
                if (!mix) {
            ...
            if (light && (right != null)) {
                ...
                if (output == null || output.Length < numsamples)
                    output = new float[numsamples];

                if (right != null) {
                    if (!mix)
                        Array.Clear(right, 0, right.Length);
                    allpassR[0].processReplace(input, output);
                    for (int i = 1; i < allpassR.Length; i++)
                        allpassR[i].processReplace(output);
                    for (int i = 0; i < combR.Length; i++)
                        combR[i].processMix(output, right);
                }

                if (!mix)
                    Array.Clear(left, 0, left.Length);
                allpassL[0].processReplace(input, output);
                for (int i = 1; i < allpassL.Length; i++)
                    allpassL[i].processReplace(output);
                for (int i = 0; i < combL.Length; i++)
                    combL[i].processMix(output, left);
            }
        ...
        public void setLightMode(bool light) {
            this.light = light;
        }
    }
}
