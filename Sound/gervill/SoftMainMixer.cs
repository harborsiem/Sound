...
namespace SystemX.Media.Sound {
     ...
     */
    public sealed class SoftMainMixer {
        ...
        private class SoftChannelMixerContainer {
            internal IModelChannelMixer mixer;
            internal SoftAudioBuffer[] buffers;
        }

        public const int CHANNEL_LEFT = 0;
        public const int CHANNEL_RIGHT = 1;
        public const int CHANNEL_MONO = 2;
        public const int CHANNEL_DELAY_LEFT = 3;
        public const int CHANNEL_DELAY_RIGHT = 4;
        public const int CHANNEL_DELAY_MONO = 5;
        public const int CHANNEL_EFFECT1 = 6;
        public const int CHANNEL_EFFECT2 = 7;
        public const int CHANNEL_DELAY_EFFECT1 = 8;
        public const int CHANNEL_DELAY_EFFECT2 = 9;
        ...
        internal bool active_sensing_on = false;
        private long msec_last_activity = -1;
        private bool pusher_silent = false;
        private int pusher_silent_count = 0;
        private long sample_pos = 0;
        internal bool readfully = true;
        ...
        private readonly SoftReverb reverb;
        private readonly ISoftAudioProcessor chorus;
        private readonly ISoftAudioProcessor agc;
        private long msec_buffer_len = 0;
        private int buffer_len = 0;
        internal IDictionary<Int64, Object> midimessages = new Dictionary<Int64, Object>(); //TreeMap<Int64, Object>
        private int delay_midievent = 0;
        private int max_delay_midievent = 0;
        internal double last_volume_left = 1.0;
        internal double last_volume_right = 1.0;
        private readonly double[] co_master_balance = new double[1];
        private readonly double[] co_master_volume = new double[1];
        private readonly double[] co_master_coarse_tuning = new double[1];
        private readonly double[] co_master_fine_tuning = new double[1];
        private readonly AudioInputStream ais;
        private HashSet<SoftChannelMixerContainer> registeredMixers = null;
        private HashSet<IModelChannelMixer> stoppedMixers = null;
        private SoftChannelMixerContainer[] cur_registeredMixers = null;
        internal ISoftControl co_master; // = new SoftControlImpl0();

        private class SoftControlImpl0 : ISoftControl {
            ...
            internal SoftControlImpl0(SoftMainMixer caller) {
                balance = caller.co_master_balance;
                volume = caller.co_master_volume;
                coarse_tuning = caller.co_master_coarse_tuning;
                fine_tuning = caller.co_master_fine_tuning;
            }
                            ...
                            case 0x04: // Device Control
                                ...
                                switch (subid2) {
                                    ...
                                    case 0x05: // Global Parameter Control
                                        ...
                                        long[] parms = new long[paramCount];
                                        ...
                                        globalParameterControlChange(slotPath, parms, values);
                                        break;
                                    default:
                                        break;
                                }
        ...
        private void processMessages(long timeStamp) {
            Dictionary<Int64, Object> tmpMessages = new Dictionary<long, object>(midimessages);
            Dictionary<Int64, Object>.Enumerator iter = tmpMessages.GetEnumerator(); //.entrySet().iterator();
            while (iter.MoveNext()) {
                KeyValuePair<Int64, Object> entry = iter.Current;
                if (entry.Key >= (timeStamp + msec_buffer_len))
                    return;
                long msec_delay = entry.Key - timeStamp;
                ...
                midimessages.Remove(entry.Key); // iter.Remove();
            }
            delay_midievent = 0;
        }

        internal void processAudioBuffers() {
            if (synth.weakstream != null && synth.weakstream.ReadSilentSamples() != 0) {
                sample_pos += synth.weakstream.ReadSilentSamples();
                synth.weakstream.WriteSilentSamples(0);
            }
            ...
            lock (control_mutex) {
                ...
                if (active_sensing_on) {
                    ...
                    if ((msec_pos - msec_last_activity) > 1000000) {
                        ...
                        foreach (SoftChannel c in synth.channels)
                            c.allSoundOff();
                    }
                ...
                if (cur_registeredMixers == null) {
                    if (registeredMixers != null) {
                        cur_registeredMixers =
                                new SoftChannelMixerContainer[registeredMixers.Count];
                        registeredMixers.CopyTo(cur_registeredMixers);
                    }
                }
            ...
            if (act_registeredMixers != null) {
                ...
                foreach (SoftChannelMixerContainer cmixer in act_registeredMixers) {
            ...
            if (buffers[CHANNEL_LEFT].isSilent()
                ...
                lock (control_mutex) {
                    midimessages_size = midimessages.Count;
                }
        ...
        // Must only be called within control_mutex synchronization
        public void activity() {
            ...
            if (pusher_silent) {
                ...
                if (synth.weakstream != null) {
                    ...
                    silent_samples = synth.weakstream.ReadSilentSamples();
                }
            }
        ...
        public void stopMixer(IModelChannelMixer mixer) {
            if (stoppedMixers == null)
                stoppedMixers = new HashSet<IModelChannelMixer>();
            stoppedMixers.Add(mixer);
        }

        public void registerMixer(IModelChannelMixer mixer) {
            if (registeredMixers == null)
        ...
        public SoftMainMixer(SoftSynthesizer synth) {
            co_master = new SoftControlImpl0(this);
            this.synth = synth;
            ...
            ais = new AudioInputStream(input, synth.getFormat(), AudioSystem.NOT_SPECIFIED);
        }
        ...
        public void globalParameterControlChange(int[] slothpath, long[] parms,
                long[] paramsvalue) {
            ...
            lock (control_mutex) {
                ...
                if (slothpath[0] == 0x01 * 128 + 0x01) {
                    for (int i = 0; i < paramsvalue.Length; i++) {
                        reverb.globalParameterControlChange(slothpath, parms[i],
                                paramsvalue[i]);
                    }
                }
                if (slothpath[0] == 0x01 * 128 + 0x02) {
                    for (int i = 0; i < paramsvalue.Length; i++) {
                        chorus.globalParameterControlChange(slothpath, parms[i],
                                paramsvalue[i]);
                    }
                }
            }
        ...
        public void processMessage(Object object0) {
            if (object0 is byte[])
                processMessage((byte[])object0);
            if (object0 is MidiMessage)
                processMessage((MidiMessage)object0);
        }
        ...
        public void processMessage(int ch, int cmd, int data1, int data2) {
            ...
            switch (cmd) {
                ...
                case ShortMessage.PITCH_BEND:
                    softchannel.setPitchBend(data1 + (byte)data2 * 128);
                    break;
                default:
                    break;
            }
        ...
        public long getMicrosecondPosition() {
            if (pusher_silent) {
                if (synth.weakstream != null) {
                    return (long)((sample_pos + synth.weakstream.ReadSilentSamples())
                            * (1000000.0 / samplerate));
                }
            }
        ...
        private class InputStreamImpl : InputStream {
            SoftMainMixer caller;
            private readonly SoftAudioBuffer[] buffers; // = SoftMainMixer.this.buffers;
            private readonly int nrofchannels;
            private readonly int buffersize; // = buffers[0].getSize();
            private readonly byte[] bbuffer;
            private int bbuffer_pos = 0;
            private readonly byte[] single = new byte[1];

            public InputStreamImpl(SoftMainMixer caller) {
                this.caller = caller;
                buffers = caller.buffers;
                nrofchannels = caller.synth.getFormat().getChannels();
                buffersize = buffers[0].getSize();
                bbuffer = new byte[buffersize
                    * (caller.synth.getFormat()
