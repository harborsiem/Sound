 ...
 */
    public sealed class SF2Instrument : ModelInstrument {

        internal String name = "";
        internal int preset = 0;
        internal int bank = 0;
        internal long library = 0;
        internal long genre = 0;
        internal long morphology = 0;
        internal SF2GlobalRegion globalregion = null;
        internal IList<SF2InstrumentRegion> regions = new List<SF2InstrumentRegion>();
        ...
        public override Patch getPatch() {
            if (bank == 128)
        ...
        public IList<SF2InstrumentRegion> getRegions() {
            return regions;
        }
        ...
        public override ModelPerformer[] getPerformers() {
            ...
            foreach (SF2InstrumentRegion presetzone in regions)
                performercount += presetzone.getLayer().getRegions().Count;
            ModelPerformer[] performers = new ModelPerformer[performercount];
            int pi = 0;

            SF2GlobalRegion presetglobal = globalregion;
            foreach (SF2InstrumentRegion presetzone in regions) {
                Dictionary<Int32, Int16> pgenerators = new Dictionary<Int32, Int16>();
                foreach (KeyValuePair<Int32, Int16> pair in presetzone.getGenerators()) {
                    pgenerators[pair.Key] = pair.Value;
                }
                //pgenerators.putAll(presetzone.getGenerators());
                if (presetglobal != null)
                    foreach (KeyValuePair<Int32, Int16> pair in presetglobal.getGenerators()) {
                        pgenerators[pair.Key] = pair.Value;
                    }
                ...
                foreach (SF2LayerRegion layerzone in layer.getRegions()) {
                    ...
                    Dictionary<Int32, Int16> generators = new Dictionary<Int32, Int16>();
                    if (layerglobal != null) {
                        foreach (KeyValuePair<Int32, Int16> pair in layerglobal.getGenerators()) {
                            generators[pair.Key] = pair.Value;
                        }
                        //generators.putAll(layerglobal.getGenerators());
                    }
                    foreach (KeyValuePair<Int32, Int16> pair in layerzone.getGenerators()) {
                        generators[pair.Key] = pair.Value;
                    }
                    //generators.putAll(layerzone.getGenerators());
                    foreach (KeyValuePair<Int32, Int16> gen in pgenerators) {
                        short val;
                        if (!generators.ContainsKey(gen.Key))
                            val = layerzone.getShort(gen.Key);
                        else
                            val = generators[gen.Key];
                        val += gen.Value;
                        generators[gen.Key] = val;
                    }
                    ...
                    if (volHold != -12000) {
                        ...
                        volHold += (short)(60 * volKeyNumToHold);
                    ...
                    if (volDecay != -12000) {
                        ...
                        volDecay += (short)(60 * volKeyNumToDecay);
                    ...
                    if (getGeneratorValue(generators,
                        ...
                        if (modHold != -12000) {
                            ...
                            modHold += (short)(60 * modKeyNumToHold);
                            float fvalue = -modKeyNumToHold * 128;
                            ModelIdentifier src = ModelSource.SOURCE_NOTEON_KEYNUMBER;
                            dest = ModelDestination.DESTINATION_EG2_HOLD;
                            performer.getConnectionBlocks().Add(
                        ...
                        if (modDecay != -12000) {
                            ...
                            modDecay += (short)(60 * modKeyNumToDecay);
                        ...
                        if (getGeneratorValue(generators,
                            ...
                            dest = ModelDestination.DESTINATION_FILTER_FREQ;
                            performer.getConnectionBlocks().Add(
                                new ModelConnectionBlock(new ModelSource(src),
                                    fvalue, new ModelDestination(dest)));
                        }
                    ...
                    if (getGeneratorValue(generators,
                        ...
                        ModelIdentifier src = ModelSource.SOURCE_LFO1;
                        dest = ModelDestination.DESTINATION_GAIN;
                        performer.getConnectionBlocks().Add(
                            new ModelConnectionBlock(
                                new ModelSource(src,
                                    ModelStandardTransform.DIRECTION_MIN2MAX,
                    ...
                    if (layer.getGlobalRegion() != null) {
                        foreach (SF2Modulator modulator
                                in layer.getGlobalRegion().getModulators()) {
                            convertModulator(performer, modulator);
                        }
                    }
                    foreach (SF2Modulator modulator in layerzone.getModulators())
                        convertModulator(performer, modulator);

                    if (presetglobal != null) {
                        foreach (SF2Modulator modulator in presetglobal.getModulators())
                            convertModulator(performer, modulator);
                    }
                    foreach (SF2Modulator modulator in presetzone.getModulators())
                        convertModulator(performer, modulator);
        ...
        private static ModelSource convertSource(int src) {
            ...
            if ((src & SF2Modulator.SOURCE_MIDI_CONTROL) != 0) {
                id = new ModelIdentifier("midi_cc", (idsrc.ToString(CultureInfo.InvariantCulture)));
            } else {
        ...
        internal static ModelDestination convertDestination(int dst,
        ...
        private void addTimecentValue(ModelPerformer performer,
            ...
            double fvalue;
            if (value == -12000)
                fvalue = Double.NegativeInfinity;
            else
                fvalue = value;
            performer.getConnectionBlocks().Add(
        ...
        private short getGeneratorValue(IDictionary<Int32, Int16> generators, int gen) {
