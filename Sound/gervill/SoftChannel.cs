 ...
 */
    public sealed class SoftChannel : IMidiChannel, IModelDirectedPlayer {

        private static readonly bool[] dontResetControls = new bool[128];
        static SoftChannel() {
        ...
        private const int RPN_NULL_VALUE = (127 << 7) + 127;
        private int rpn_control = RPN_NULL_VALUE;
        private int nrpn_control = RPN_NULL_VALUE;
        internal double portamento_time = 1; // keyschanges per control buffer time
        internal int[] portamento_lastnote = new int[128];
        internal int portamento_lastnote_ix = 0;
        private bool portamento = false;
        private bool mono = false;
        private bool mute = false;
        private bool solo = false;
        private bool solomute = false;
        private readonly Object control_mutex;
        private readonly int channel;
        private readonly SoftVoice[] voices;
        private int bank;
        private int program;
        private readonly SoftSynthesizer synthesizer;
        private readonly SoftMainMixer mainmixer;
        private readonly int[] polypressure = new int[128];
        private int channelpressure = 0;
        private readonly int[] controller = new int[128];
        private int pitchbend;
        private readonly double[] co_midi_pitch = new double[1];
        private readonly double[] co_midi_channel_pressure = new double[1];
        internal SoftTuning tuning = new SoftTuning();
        internal int tuning_bank = 0;
        internal int tuning_program = 0;
        internal SoftInstrument current_instrument = null;
        internal IModelChannelMixer current_mixer = null;
        internal IModelDirector current_director = null;

        // Controller Destination Settings
        internal int cds_control_number = -1;
        internal ModelConnectionBlock[] cds_control_connections = null;
        internal ModelConnectionBlock[] cds_channelpressure_connections = null;
        internal ModelConnectionBlock[] cds_polypressure_connections = null;
        internal bool sustain = false;
        internal bool[][] keybasedcontroller_active = null;
        internal double[][] keybasedcontroller_value = null;

        private class MidiControlObject : ISoftControl {
            double[] pitch; // = co_midi_pitch;
            double[] channel_pressure; // = co_midi_channel_pressure;
            double[] poly_pressure = new double[1];

            public MidiControlObject(SoftChannel caller) {
                pitch = caller.co_midi_pitch;
                channel_pressure = caller.co_midi_channel_pressure;
            }
        ...
        private readonly ISoftControl[] co_midi = new ISoftControl[128];
        //{
        ...
        private readonly double[][] co_midi_cc_cc = new double[128][]; //[128][1]
        private readonly ISoftControl co_midi_cc; // = new SoftControlImpl1();
        private class SoftControlImpl1 : ISoftControl {
            double[][] cc; // = co_midi_cc_cc;
            public SoftControlImpl1(SoftChannel caller) {
                cc = caller.co_midi_cc_cc;
            }

            public double[] get(int instance, String name) {
                if (name == null)
                    return null;
                return cc[Int32.Parse(name, NumberFormatInfo.InvariantInfo)];
            }
        }
        Dictionary<Int32, int[]> co_midi_rpn_rpn_i = new Dictionary<Int32, int[]>();
        Dictionary<Int32, double[]> co_midi_rpn_rpn = new Dictionary<Int32, double[]>();
        private readonly ISoftControl co_midi_rpn; // = new SoftControlImpl2();
        private class SoftControlImpl2 : ISoftControl {
            Dictionary<Int32, double[]> rpn; // = co_midi_rpn_rpn;
            public SoftControlImpl2(SoftChannel caller) {
                rpn = caller.co_midi_rpn_rpn;
            }

            public double[] get(int instance, String name) {
                if (name == null)
                    return null;
                int iname = Int32.Parse(name, NumberFormatInfo.InvariantInfo);
                double[] v = null;
                if (rpn.ContainsKey(iname)) {
                    v = rpn[iname];
                }
                if (v == null) {
                    v = new double[1];
        ...
        Dictionary<Int32, int[]> co_midi_nrpn_nrpn_i = new Dictionary<Int32, int[]>();
        Dictionary<Int32, double[]> co_midi_nrpn_nrpn = new Dictionary<Int32, double[]>();
        private readonly ISoftControl co_midi_nrpn; // = new SoftControlImpl3();
        internal class SoftControlImpl3 : ISoftControl {
            Dictionary<Int32, double[]> nrpn; // = co_midi_nrpn_nrpn;
            public SoftControlImpl3(SoftChannel caller) {
                nrpn = caller.co_midi_nrpn_nrpn;
            }
        ...
        public SoftChannel(SoftSynthesizer synth, int channel) {
            for (int i = 0; i < co_midi.Length; i++) {
                co_midi[i] = new MidiControlObject(this);
            }
            for (int i = 0; i < co_midi_cc_cc.Length; i++) {
                co_midi_cc_cc[i] = new double[1];
            }

            co_midi_cc = new SoftControlImpl1(this);
            co_midi_rpn = new SoftControlImpl2(this);
            co_midi_nrpn = new SoftControlImpl3(this);
        ...
        internal void initVoice(SoftVoice voice, SoftPerformer p, int voiceID,
                int noteNumber, int velocity, int delay, ModelConnectionBlock[] connectionBlocks,
                IModelChannelMixer channelmixer, bool releaseTriggered) {
            if (voice.active) {
            ...
            voice.performer = p;
            voice.objects.Clear();
            voice.objects["midi"] = co_midi[noteNumber];
            voice.objects["midi_cc"] = co_midi_cc;
            voice.objects["midi_rpn"] = co_midi_rpn;
            voice.objects["midi_nrpn"] = co_midi_nrpn;
        ...
        public void play(int performerIndex, ModelConnectionBlock[] connectionBlocks) {
            ...
            bool releasetriggered = play_releasetriggered;
        ...
        internal void applyInstrumentCustomization() {
            ...
            for (int i = 0; i < new_performers.Length; i++) {
                ...
                ((List<IModelOscillator>)new_performer.getOscillators()).AddRange(performer.getOscillators());
                ((List<ModelConnectionBlock>)new_performer.getConnectionBlocks()).AddRange(
                        performer.getConnectionBlocks());
                new_performers[i] = new_performer;

                IList<ModelConnectionBlock> connblocks =
                        new_performer.getConnectionBlocks();

                if (cds_control_connections != null) {
                    String cc = (cds_control_number.ToString(CultureInfo.InvariantCulture));
                    List<ModelConnectionBlock> tmpblocks = new List<ModelConnectionBlock>(connblocks); //a@
                    IEnumerator<ModelConnectionBlock> iter = tmpblocks.GetEnumerator();
                    while (iter.MoveNext()) {
                        ModelConnectionBlock conn = iter.Current;
                        ...
                        if (removeok)
                            connblocks.Remove(conn); //iter.Remove();
                    }
        ...
        private ModelConnectionBlock[] createModelConnections(ModelIdentifier sid,
            ...
            */
            ...
            List<ModelConnectionBlock> conns = new List<ModelConnectionBlock>();
            ...
            for (int i = 0; i < destination.Length; i++) {
                ...
                if (d == 2) {
                    double scale = (r / 64.0);
                    IModelTransform mt = new ModelTransformImpl1(scale);
                    //{
                    //    double s = scale;
                ...
                if (d == 5) {
                    ...
                    IModelTransform mt = new ModelTransformImpl2(scale);
                    //{
                    //    double s = scale;
                    //    public double transform(double value) {
        ...
        public void mapControlToDestination(int control, int[] destination, int[] range) {
            ...
            cds_control_connections
                    = createModelConnections(
                        new ModelIdentifier("midi_cc", (control.ToString(CultureInfo.InvariantCulture))),
                        destination, range);
        }
        ...
        public void controlChangePerNote(int noteNumber, int controller, int value) {
            ...
            */
            ...
            if (keybasedcontroller_active == null) {
                keybasedcontroller_active = new bool[128][];
                keybasedcontroller_value = new double[128][];
            }
            if (keybasedcontroller_active[noteNumber] == null) {
                keybasedcontroller_active[noteNumber] = new bool[128];
                ...
                switch (controller) {
                    ...
                    case 97:
                        ...
                        if (nrpn_control != RPN_NULL_VALUE) {
                            bool bVal_i = co_midi_nrpn_nrpn_i.ContainsKey(nrpn_control);
                            int[] val_i = null;
                            if (bVal_i) {
                                val_i = co_midi_nrpn_nrpn_i[nrpn_control];
                            }
                            if (val_i != null)
                                val = val_i[0];
                        }
                        if (rpn_control != RPN_NULL_VALUE) {
                            bool bVal_i = co_midi_rpn_rpn_i.ContainsKey(rpn_control);
                            int[] val_i = null;
                            if (bVal_i) {
                                val_i = co_midi_rpn_rpn_i[rpn_control];
                            }
                            if (val_i != null)
                                val = val_i[0];
                        }
        ...
        public void nrpnChange(int controller, int value) {
            ...
            bool bVal_i = co_midi_nrpn_nrpn_i.ContainsKey(controller);
            bool bVal_d = co_midi_nrpn_nrpn.ContainsKey(controller);
            int[] val_i = null;
            if (bVal_i) {
                val_i = co_midi_nrpn_nrpn_i[controller];
            }
            double[] val_d = null;
            if (bVal_d) {
                val_d = co_midi_nrpn_nrpn[controller];
            }
            if (val_i == null) {
                val_i = new int[1];
                co_midi_nrpn_nrpn_i[controller] = val_i;
            }
            if (val_d == null) {
                val_d = new double[1];
                co_midi_nrpn_nrpn[controller] = val_d;
            }
            val_i[0] = value;
        ...
        public void rpnChange(int controller, int value) {
            ...
            bool bVal_i = co_midi_rpn_rpn_i.ContainsKey(controller);
            bool bVal_d = co_midi_rpn_rpn.ContainsKey(controller);
            int[] val_i = null;
            if (bVal_i) {
                val_i = co_midi_rpn_rpn_i[controller];
            }
            double[] val_d = null;
            if (bVal_d) {
                val_d = co_midi_rpn_rpn[controller];
            }
            if (val_i == null) {
                val_i = new int[1];
                co_midi_rpn_rpn_i[controller] = val_i;
            }
            if (val_d == null) {
                val_d = new double[1];
                co_midi_rpn_rpn[controller] = val_d;
            }
            val_i[0] = value;
            val_d[0] = val_i[0] * (1.0 / 16384.0);
        ...
        public void resetAllControllers(bool allControls) {
            lock (control_mutex) {
                mainmixer.activity();
                ...
                if (allControls) {
                    ...
                    foreach (int controller in co_midi_rpn_rpn.Keys) {
                        // don't reset tuning settings
                        if (controller != 3 && controller != 4)
                            rpnChange(controller, 0);
                    }
                    foreach (int controller in co_midi_nrpn_nrpn.Keys)
        ...
        public void setSolo(bool soloState) {
            ...
            lock (control_mutex) {
                ...
                foreach (SoftChannel c in synthesizer.channels) {
                    if (c.solo) {
                        soloinuse = true;
                        break;
                    }
        ...
        private void setSoloMute(bool mute) {
            lock (control_mutex) {
                if (solomute == mute)
                    return;
                this.solomute = mute;
        ...
        private class ModelTransformImpl2 : IModelTransform {
            double s; // = scale;
            public ModelTransformImpl2(double scale) {
                s = scale;
            }
            public double transform(double value) {
                return -((5.0 / 12.0) / Math.Log(10))
