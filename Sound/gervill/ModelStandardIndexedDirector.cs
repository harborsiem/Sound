 ...
 */
    public sealed class ModelStandardIndexedDirector : IModelDirector {

        private readonly ModelPerformer[] performers;
        private readonly IModelDirectedPlayer player;
        private bool noteOnUsed = false;
        private bool noteOffUsed = false;

        // Variables needed for index
        private sbyte[][] trantables;
        private int[] counters;
        private int[][] mat;

        public ModelStandardIndexedDirector(ModelPerformer[] performers,
                IModelDirectedPlayer player) {
            ModelPerformer[] temp = new ModelPerformer[performers.Length];
            Array.Copy(performers, temp, performers.Length);
            this.performers = temp;
            this.player = player;
            foreach (ModelPerformer p in performers) {
        ...
        private void buildindex() {
            trantables = new sbyte[2][];
            for (int i = 0; i < trantables.Length; i++) {
                trantables[i] = new sbyte[129];
            }
            counters = new int[trantables.Length];
            foreach (ModelPerformer performer in performers) {
