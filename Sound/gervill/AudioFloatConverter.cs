 ...
 */
    public abstract class AudioFloatConverter {
        ...
        private class AudioFloatLSBFilter : AudioFloatConverter {
            ...
            internal AudioFloatLSBFilter(AudioFloatConverter converter, AudioFormat format) {
            ...
            public override byte[] toByteArray(float[] in_buff, int in_offset, int in_len,
            ...
            public override float[] toFloatArray(byte[] in_buff, int in_offset,
                                                 float[] out_buff, int out_offset, int out_len) {
                if (mask_buffer == null || mask_buffer.Length < in_buff.Length)
                    mask_buffer = new byte[in_buff.Length];
                Array.Copy(in_buff, 0, mask_buffer, 0, in_buff.Length);
        ...
        private class AudioFloatConversion64L : AudioFloatConverter {

            internal ByteBuffer bytebuffer = null;

            internal DoubleBuffer floatbuffer = null;

            internal double[] double_buff = null;
            ...
            public override float[] toFloatArray(byte[] in_buff, int in_offset,
                ...
                bytebuffer.Write(in_buff, in_offset, in_len);
                if (double_buff == null
                        || double_buff.Length < out_len + out_offset)
                    double_buff = new double[out_len + out_offset];
                floatbuffer.Read(double_buff, out_offset, out_len);
            ...
            public override byte[] toByteArray(float[] in_buff, int in_offset, int in_len,
                ...
                floatbuffer.Write(double_buff, in_offset, in_len);
                bytebuffer.Read(out_buff, out_offset, out_len);
                return out_buff;
            }
        }

        // PCM 64 bit float, big-endian
        private class AudioFloatConversion64B : AudioFloatConverter {
            internal ByteBuffer bytebuffer = null;
        ...
        private class AudioFloatConversion32L : AudioFloatConverter {
            internal ByteBuffer bytebuffer = null;

            internal SingleBuffer floatbuffer = null;
            ...
            public override float[] toFloatArray(byte[] in_buff, int in_offset,
                ...
                if (bytebuffer == null || bytebuffer.Capacity() < in_len) {
                    bytebuffer = ByteBuffer.Allocate(in_len).Order(
                            ByteOrder.LittleEndian);
                    floatbuffer = bytebuffer.AsSingleBuffer();
                }
                bytebuffer.Position(0);
                floatbuffer.Position(0);
                bytebuffer.Write(in_buff, in_offset, in_len);
                floatbuffer.Read(out_buff, out_offset, out_len);
                return out_buff;
            }
            ...
            public override byte[] toByteArray(float[] in_buff, int in_offset, int in_len,
                ...
                floatbuffer.Position(0);
                bytebuffer.Position(0);
                floatbuffer.Write(in_buff, in_offset, in_len);
                bytebuffer.Read(out_buff, out_offset, out_len);
                return out_buff;
            }
        }
        ...
        private class AudioFloatConversion32B : AudioFloatConverter {
            internal ByteBuffer bytebuffer = null;
        ...
        // PCM 8 bit, signed
        private class AudioFloatConversion8S : AudioFloatConverter {
            public override float[] toFloatArray(byte[] in_buff, int in_offset,
                                                 float[] out_buff, int out_offset, int out_len) {
                int ix = in_offset;
                int ox = out_offset;
        ...
        // PCM 8 bit, unsigned
        private class AudioFloatConversion8U : AudioFloatConverter {
            public override float[] toFloatArray(byte[] in_buff, int in_offset,
                                                 float[] out_buff, int out_offset, int out_len) {
                int ix = in_offset;
                int ox = out_offset;
        ...
        private class AudioFloatConversion16SL : AudioFloatConverter {
            public override float[] toFloatArray(byte[] in_buff, int in_offset,
                                                 float[] out_buff, int out_offset, int out_len) {
                int ix = in_offset;
        ...
        private class AudioFloatConversion16SB : AudioFloatConverter {
            public override float[] toFloatArray(byte[] in_buff, int in_offset,
                                                 float[] out_buff, int out_offset, int out_len) {
                int ix = in_offset;
                int ox = out_offset;
        ...
        private class AudioFloatConversion16UL : AudioFloatConverter {
            public override float[] toFloatArray(byte[] in_buff, int in_offset,
                                                 float[] out_buff, int out_offset, int out_len) {
                int ix = in_offset;
                int ox = out_offset;
        ...
        private class AudioFloatConversion16UB : AudioFloatConverter {
            public override float[] toFloatArray(byte[] in_buff, int in_offset,
                                                 float[] out_buff, int out_offset, int out_len) {
                int ix = in_offset;
                int ox = out_offset;
        ...
        private class AudioFloatConversion24SL : AudioFloatConverter {
            public override float[] toFloatArray(byte[] in_buff, int in_offset,
                                                 float[] out_buff, int out_offset, int out_len) {
                int ix = in_offset;
                int ox = out_offset;
        ...
        private class AudioFloatConversion24SB : AudioFloatConverter {
            public override float[] toFloatArray(byte[] in_buff, int in_offset,
                                                 float[] out_buff, int out_offset, int out_len) {
                int ix = in_offset;
                int ox = out_offset;
        ...
        private class AudioFloatConversion24UL : AudioFloatConverter {
            public override float[] toFloatArray(byte[] in_buff, int in_offset,
                                                 float[] out_buff, int out_offset, int out_len) {
                int ix = in_offset;
                int ox = out_offset;
        ...
        private class AudioFloatConversion24UB : AudioFloatConverter {
            public override float[] toFloatArray(byte[] in_buff, int in_offset,
                                                 float[] out_buff, int out_offset, int out_len) {
                int ix = in_offset;
                int ox = out_offset;
        ...
        private class AudioFloatConversion32SL : AudioFloatConverter {
            public override float[] toFloatArray(byte[] in_buff, int in_offset,
                                                 float[] out_buff, int out_offset, int out_len) {
                int ix = in_offset;
                int ox = out_offset;
        ...
        private class AudioFloatConversion32SB : AudioFloatConverter {
            public override float[] toFloatArray(byte[] in_buff, int in_offset,
                                                 float[] out_buff, int out_offset, int out_len) {
                int ix = in_offset;
                int ox = out_offset;
        ...
        private class AudioFloatConversion32UL : AudioFloatConverter {
            public override float[] toFloatArray(byte[] in_buff, int in_offset,
                                                 float[] out_buff, int out_offset, int out_len) {
                int ix = in_offset;
                int ox = out_offset;
        ...
        private class AudioFloatConversion32UB : AudioFloatConverter {
        ...
        // PCM 32+ bit, signed, little-endian
        private class AudioFloatConversion32xSL : AudioFloatConverter {

            private readonly int xbytes;

            internal AudioFloatConversion32xSL(int xbytes) {
                this.xbytes = xbytes;
            }
        ...
        // PCM 32+ bit, signed, big-endian
        private class AudioFloatConversion32xSB : AudioFloatConverter {

            private readonly int xbytes;

            internal AudioFloatConversion32xSB(int xbytes) {
                this.xbytes = xbytes;
            }
        ...
        // PCM 32+ bit, unsigned, little-endian
        private class AudioFloatConversion32xUL : AudioFloatConverter {

            private readonly int xbytes;

            internal AudioFloatConversion32xUL(int xbytes) {
                this.xbytes = xbytes;
            }
        ...
        // PCM 32+ bit, unsigned, big-endian
        private class AudioFloatConversion32xUB : AudioFloatConverter {

            private readonly int xbytes;

            internal AudioFloatConversion32xUB(int xbytes) {
                this.xbytes = xbytes;
            }
        ...
        public byte[] toByteArray(float[] in_buff, int in_len,
            byte[] out_buff, int out_offset) {
