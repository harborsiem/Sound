 ...
 */
    public sealed class SoftChorus : ISoftAudioProcessor {
        ...
        private class VariableDelay {
            ...
            internal VariableDelay(int maxbuffersize) {
            ...
            public void processMix(float[] input, float[] output, float[] rout) {
                ...
                if (rout == null)
                    for (int i = 0; i < len; i++) {
                        ...
                        float o = a * (1 - s) + b * (s);
                        output[i] += o * gain;
                        delaybuffer[rovepos] = input[i] + o * feedback;
            ...
            public void processReplace(float[] input, float[] output, float[] rout) {
                Array.Clear(output, 0, output.Length);
                Array.Clear(rout, 0, rout.Length);
                processMix(input, output, rout);
            }
        }
        ...
        private class LFODelay {
            ...
            private readonly double controlrate;

            internal LFODelay(double samplerate, double controlrate) {
            ...
            public void processMix(float[] input, float[] output, float[] rout) {
                ...
                vdelay.processMix(input, output, rout);
            }
            ...
            public void processReplace(float[] input, float[] output, float[] rout) {
                ...
                while (phase > (Math.PI * 2)) phase -= (Math.PI * 2);
                vdelay.setDelay((float)(depth * 0.5 * (Math.Cos(phase) + 2)));
                vdelay.processReplace(input, output, rout);
