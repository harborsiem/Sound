 ...
 */
    public sealed class DLSInstrument : ModelInstrument {

        internal int preset = 0;
        internal int bank = 0;
        internal bool druminstrument = false;
        internal byte[] guid = null;
        internal DLSInfo info = new DLSInfo();
        internal IList<DLSRegion> regions = new List<DLSRegion>();
        internal IList<DLSModulator> modulators = new List<DLSModulator>();
        ...
        public IList<DLSRegion> getRegions() {
            return regions;
        }

        public IList<DLSModulator> getModulators() {
            return modulators;
        }
        ...
        private ModelConnectionBlock convertToModel(DLSModulator mod) {
            ...
            if (scale == Int32.MinValue)
                f_scale = Double.NegativeInfinity;
            else
        ...
        public override ModelPerformer[] getPerformers() {
            List<ModelPerformer> performers = new List<ModelPerformer>();

            Dictionary<String, DLSModulator> modmap = new Dictionary<String, DLSModulator>();
            foreach (DLSModulator mod in getModulators()) {
                modmap[mod.getSource() + "x" + mod.getControl() + "=" +
                        mod.getDestination()] = mod;
            }

            Dictionary<String, DLSModulator> insmodmap = new Dictionary<String, DLSModulator>();

            foreach (DLSRegion zone in regions) {
                ...
                insmodmap.Clear();
                foreach (KeyValuePair<String, DLSModulator> pair in modmap) {
                    insmodmap[pair.Key] = pair.Value;
                }
                //insmodmap.putAll(modmap);
                foreach (DLSModulator mod in zone.getModulators()) {
                    insmodmap[mod.getSource() + "x" + mod.getControl() + "=" +
                            mod.getDestination()] = mod;
                }

                IList<ModelConnectionBlock> blocks = performer.getConnectionBlocks();
                foreach (DLSModulator mod in insmodmap.Values) {
                ...
                if (sampleopt.getLoops().Count != 0) {
                    DLSSampleLoop loop = sampleopt.getLoops()[0];
                    osc.setLoopStart((int)loop.getStart());
                    osc.setLoopLength((int)loop.getLength());
                    if (loop.getLoopType() == DLSSampleLoop.LOOP_TYPE_FORWARD)
                        osc.setLoopType(ModelWavetable.LOOP_TYPE_FORWARD);
                    if (loop.getLoopType() == DLSSampleLoop.LOOP_TYPE_RELEASE)
        ...
        public byte[] getGuid() {
            return guid == null ? null : (byte[])guid.Clone();
        }

        public void setGuid(byte[] guid) {
            this.guid = guid == null ? null : (byte[])guid.Clone();
        }
    }
}
